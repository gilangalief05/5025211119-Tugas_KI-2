...
def generate_round_keys(binary_representation_key):
...
def encryption(plain_str):
    ...
    # print("Final sub cipher bin:", final_cipher_str, "\nFinal sub cipher:", final_cipher_ascii, "\n")
    return final_cipher_ascii


# Decrypt cipher
def decryption(enc):
    final_cipher = str_to_bin(enc)
...
def decrypt_str(enc):
    ...
    sub_str = [(enc[i:i+8]) for i in range(0, len(enc), 8)]
    for sub in sub_str:
        dec += decryption(sub)
...
# Test only
# enc = encrypt_str("Lorem ipsum dolor sit amet, consectetur adipiscing elit.")
# dec = decrypt_str(enc)